...
// Import the restaurant UI examples
import { RESTAURANT_UI_EXAMPLES } from './a2ui_examples.js';

// The A2UI schema remains constant for all A2UI responses.
// We construct it as a properly escaped JSON string
const A2UI_SCHEMA = JSON.stringify({
 ...
 *
 * @param {string} base_url - The base URL for resolving static assets like logos.
 * @param {string} examples - A string containing the specific UI examples for the agent's task.
 * @returns {string} - A formatted string to be used as the system prompt for the LLM.
 */
function get_ui_prompt(base_url, examples) {
    // Format the examples by replacing base_url
    let formatted_examples = examples;
    if (base_url) {
        // This is a simple replacement - in a real application, you might want to use a more robust template engine
